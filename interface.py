...
def create_new_user_action() -> None:
    ...
    items = [
        {
            "message": f"Introduce an alphanumeric\
                  name of {USERNAME_LENGTH} characters: ",
            "validation_function": is_valid_not_existing_name,
        },
        {
            "message": f"Introduce an alphanumeric \
                password of {PASSWORD_LENGTH} characters: ",
...
def create_taskAction() -> None:
    ...
    items = [
        {
            "message": f"Introduce the task \
                Title ({TASK_TITLE_LENGTH}): ",
            "validation_function": is_valid_title,
        },
        {
            "message": f"Introduce the task \
                Description ({TASK_DESCRIPTION_LENGTH}): ",
...
def edit_task_action() -> None:
    ...
    items = [
        {
            "message": "Introduce the task UUID to edit: ",
        ...
        {
            "message": f"Introduce the task Description \
                ({TASK_DESCRIPTION_LENGTH}): ",
            "validation_function": is_valid_description,
        },
        {
            "message": f"Introduce the task status \
                ({TASK_STATUSES['todo']}: t, {TASK_STATUSES['pending']}: p,\
...
def main_menu() -> None:
    ...
    while True:
        ...
        print(
            f"""
        Task Manager 20000 {user_loged_line}
        Choose an option:\n{generate_menu()}
        """
        )
        selected_option = input()
        if (
            selected_option.isdigit()
            and int(selected_option) >= 1
            and int(selected_option) <= 8
        ):
