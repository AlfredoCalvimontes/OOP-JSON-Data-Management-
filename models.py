...
from error_management.exceptions import UserError, TaskError, TaskSetError, UserSetError
...
class DataEntity:
    ...
    and updated them dynamically too.
    It's used to load and contain corresponding JSON objects defined in the filepath.
    '''
    filepath: str = ""
    ...
    def udpate(self, json: Dict) -> None:
        '''Updates all the received attributes in a dictionary defined in the current class.'''
...
class User(DataEntity):
    ...
    def __init__(self,
        ...
        try:
            ...
            self._deleted: bool = deleted if deleted != None else False
...
class DataEntitySet(Set):
    ...
    def add(self, data_entity: DataEntity) -> None:
        ...
        if isinstance(data_entity, self.related_class):
            ...
            raise TypeError(f"data_entity should be type {self.related_class}.")
    ...
    def add_jSON(self, json: Dict) -> None:
        '''
        Creates an object of the related_class type using the data from the json
        providede and adds it to the DataEntitySet.
        '''
        if isinstance(json, Dict):
    ...
    @data_object_exception_manager
    def get_data_entity_by_key(self, key: str, value: str) -> Optional[DataEntity]:
        '''
        Returns an item from the DataEntitySet that has the received key with the corresponding value.
    ...
    def get_filtered_entities(self, key_values: Dict) -> Optional[Set]:
        '''
        Returns a set of related_class type that has the received attributes with their corresponding values.
        '''
        return {
    ...
    def dump(self) -> List[Dict]:
        '''Returns a list of dictionaries containing all the data in json structure.'''
        return [data_entity.get_data() for data_entity in self]

    def remove(self):
        raise TypeError("TypeError: You can't remove by value from a DataEntitySet.") 

    def discard(self):
        raise TypeError("TypeError: You can't discard by value from a DataEntitySet.") 

    def pop(self):
        raise TypeError("TypeError: You can't pop a data entity from a DataEntitySet.") 

    def clear(self):
        raise TypeError("TypeError: You can't clear the data entity set from a DataEntitySet.") 
...
class UserSet(DataEntitySet):
    ...
    def get_user_by_key(self, key: str, value: str) -> User:
        '''
        Returns the User object that has the key attribute the corresponding value.
    ...
    def user_exists(self, name: str) -> bool:
        ...
        try:
            return True if self.get_user_by_name(name) != None else False
        except Exception as e:
...
class TaskSet(DataEntitySet):
    ...
    def get_task_by_key(self, key: str, value: str) -> Task:
        '''
        Returns the Task object that has the key attribute the corresponding value.
        If it doesn't exist returns None.
        '''
        try:
            return super().get_data_entity_by_key(key, value)
    ...
    def get_user_tasks(self, owner_uuid: str, filter_status: Optional[str] = None,
                     inclue_delete: bool = False) -> Set:
        '''
    ...
    def get_last_user_created_task(self) -> Task:
        ...
        try:
            return max(self, key=lambda task: task.get_data()['creation_datetime'])
        except Exception as e:
    ...
    def update_task(self, uuid_text: str, title: str, description: str, status: str) -> None:
        ...
        try:
            ...
            datetime_now = datetime.now(timezone.utc).strftime(DATETIME_FORMAT)
            new_title = title if title != None else task_data["_title"]
            new_description = description if description != None else task_data["_description"]
            new_status = status if status != None else task_data["_status"]
