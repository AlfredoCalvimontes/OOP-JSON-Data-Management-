...
def data_object_loading(object_path: str, operation: str, data: Any = None) -> Any:
    ...
    try:
        ...
        logger.warning(f"data_object_loading: FileNotFoundError: The file at '{path}' was not found. {fnfe}")
        return None
    except json.JSONDecodeError as jse:
        logger.warning(f"data_object_loading: JSONDecodeError: The file at '{path}' is not a valid JSON file. {jse}")
        return None
    except FileError as fe:
        logger.warning(f"data_object_loading: FileError: There was an issue working with files. {fe}")
        return None
    except Exception as e:
        logger.error(f"data_object_loading: Error: An unexpected error occurred when retrieven the JSON file: {e}")
        return None
...
def get_sets_and_object() -> Dict:
    ...
    try:
        ...
        logger.error(f"Error in getSetsAndObject, returned an empty dictionary: {e}")
        return {}
...
def data_loading() -> None:
    ...
    try:
        ...
        for data_set, object in sets_and_objects.items():
            state[data_set.__name__.lower()] = data_set(data_object_loading(object.filepath, OPERATIONS['reading']))
...
def data_saving() -> None:
    ...
    try:
        ...
        # Left a separated for because it's more readable
        for data_set, object in sets_and_objects.items():
            dataset: DataEntitySet = state[data_set.__name__.lower()] # type: ignore
            data_object_loading(object.filepath, OPERATIONS['writing'], dataset.dump())
